...
- v2.5 集成（待验收部署）：显式候选画面验证、可选综合评审、视觉证据页面和安全缓存保留期；默认关闭，原文字门槛/人工边界保留。视觉策略与运行证据可追溯，模型失败或预算耗尽正常降级。

后续按 v2.5 → v2.5.5 → v2.6 顺序推进，工程测试和兼容验收逐版完成。

- v2.5 基础：候选范围内关键帧采样、时间/数量/图片预算和失败证据；现有 Codex CLI 双图输入技术验证通过。尚未接入生产视觉分析，默认文字/音频链路保持原样。
- v2.5 证据基础：Codex 显式视觉方法、严格观察结构、冻结候选附件与可选 checkpoint；新增增量视觉证据表和迁移回滚验证。尚未开放生产视觉入口，不自动重试不确定调用。

...
## 2026-09-08 获准补跑完整通过与秒级证据误拦修复（未部署）

- 用户明确允许原 29 的 4 个、原 30 的 2 个失败单元本轮各补跑一次；6 个均通过校验，确认记录与旧状态保存在隔离账本。首次补跑实际调用 29 最终评审 12 次、30 最终评审 5 次，额外调用均来自附近比较输入变化；召回与扩展全部复用。
- 29 首次补跑为 56/57，新增失败 w002_m002 并非引文越界：转写导出使用 `int(round(value))`，将短句“它不是泡面”表示为 05:39–05:39；它处在 05:21–05:41 内部。旧校验要求所有可出片证据都为正时长，错误拦截了原样引用。
- 修正为只接受严格位于整秒切片边界内部的零时长引用；任一切点上的零时长引用仍拒绝，不能据此证明整句保留。AI 原始决定、评分、引文、片段起止都不改写。3 个新增边界用例先复现 1 失败 / 2 通过，修正后定向 41 项通过，全量 1026 项通过（9 条既有警告），Ruff 与 diff 检查通过。
- 29 的该返回已在相同请求指纹下本地恢复，`reused=true`；恢复前后原始响应 SHA-256 均为 `fb7b8dba59f3cd4e9f89adba9fa43e28cc7512f315cf898d40c515a4add63e19`，未再次调用该单元。随后只重评因该结果而变化的 5 个附近单元，均成功，其余 52 个单元本地复用；没有再次确认或重复补跑不确定单元。
- 最终 29 完成 57/57，AI 决定 10 可出片、10 待审核、3 淘汰；清单 `kangxi-content-task29-recovery-20260908/manifest-20260908T113318Z.json`，结果 SHA-256 `7a12f016295c4609d6978131a9b38520e989597025f43024a69151f090deaf80`。30 完成 58/58，4 可出片、15 待审核、4 淘汰；清单 `kangxi-content-baseline-20260908/manifest-20260908T111733Z.json`，结果 SHA-256 `619af5343a8fc77bb65bbd164e2e055c62a788af61122fd0d5991f286d4db854`。
- 本轮实际 AI 调用合计 22 次，全部为最终评审：6 次获准补跑、11 次首次依赖变化重评、5 次本地恢复后的依赖变化重评。旧清单、失败凭据、调用日志和 `precision-recovery-proof.json` 均保留；不把重复计算隐去或说成只调用了 6 次。
- 最终 14 条 AI 可出片样本仍是 8–33 秒，新增“干面界李嘉欣”“泡面方便性”“佛跳墙晚餐”“十年后看旧照”已做转写与抽帧初查；完整音画人工审核尚未完成。独立预览位于 `data/diagnostics/kangxi-content-final-inspection-20260908/inspection.html`，另保留 6 个补跑单元的全部决定及初恋淘汰样本。原节目字幕与 ASR 个别错词、短片是否过碎、弱笑点及初恋是否误杀均未被掩盖。
- PR #85 保持草稿，未合并、未部署、未恢复活动正文。正式数据库、片段与发送记录未由本轮改写，模型和登录配置不变。下方此前不完整结果保留为历史过程，最新状态以本节为准。

## 2026-09-08 恢复康熙内容基线与 AI 明确决定（独立修复，未部署）

- 从正式运行 `c650128` 建立 `codex/fix-kangxi-ai-decisions`，原工作区已有改动保持原样。1 号源文件恢复历史正文修订 1；活动数据库正文未修改。
- 新康熙分析取消 3/18/12/5 数量截断与程序补时长，AI 对全部候选给出可出片、待审核或淘汰。分数不决定启用，保留原始限制意见；可出片必须有开头、主题、笑点、回应、收尾的可核验原文证据和连续起止时间。
- 新增增量迁移、明确采用确认、零条成功状态及空结果 checkpoint 恢复；旧分析只读追溯，其他选片模式不变。
- 周复盘增加单项试验和完整同素材对照审核门禁。手动修改正文记录前后快照并结束旧实验归属，历史冻结输入保留。正式效果判断继续使用已有可比样本与观察周期要求。
- 最终隔离全量回归通过 1023 项（9 条既有依赖弃用警告）；Ruff、JavaScript 语法和 diff 检查通过。代码提交 `04bae4f` 的 Linux 测试、Windows 冒烟与 Docker CI 全部通过。浏览器覆盖桌面/手机审核、单项试验导入与应用；修复异步刷新收起正在查看的证据。
- 实跑发现逐句字幕会把一句话拆成多行：证据校验现接受时间对齐、原文完整一致的连续行引用，只归一空白/标点，不补字。非法返回另存原始凭据，输入指纹和校验都一致时可在本地恢复；文字评分、伪造证据、损坏凭据和变更输入不能借此成为成功缓存，也不会自动重复计费。
- 29、30 使用只读活动库快照和相同转写进行真实 AI 对照；两项已停止，均因未完成单元而正确保留不完整状态。29 完成 53/57，已校验决定为 7 可出片、8 待审核、4 淘汰；30 完成 56/58，为 3 可出片、14 待审核、4 淘汰。数量不是整批最终结论，不允许自动切片。没有创建正式任务副本、覆盖原片段或发送记录。
- 16:21 活动库中的原 29、30 被外部操作标记删除，原目录已不存在；本轮未执行该删除，也不恢复其删除状态。用此前复制清单核验保留的复跑视频及转写 SHA-256 后，只读继续隔离分析。29 的 34 个召回/扩展单元和已完成评审保留，6 份旧校验失败凭据可本地校验，缺失凭据的请求继续等待确认。
- 详细契约、隔离重放、审片记录和正式应用步骤见 `docs/KANGXI_CONTENT_DECISIONS.md`。部署、恢复活动正文及合并需确认本次版本。
- 最终 diff 复核补充：自动流程恢复选片时保留用户已审核并取消采用的项目，不重新启用；0/2/20 数量回归同时覆盖这一人工决定。
- 真实评审纠偏：阶段说明曾把“未标注笑声/表情”泛化为待审核理由，超出了基线正文（正文也接受追问、解释、补刀形成回应闭环）。已澄清只有正文必要条件确实依赖缺失证据时才据此待审；不修改 1 号基线、不调整分数或数量。30 在当前评审落账后暂停，35 个召回/扩展单元复用，按更新的阶段说明重评。
- 整条分析恢复测试发现并修复 JSON 字段顺序造成的虚假输入变化：三个候选输入统一稳定序列化，SQLite 读回后指纹不变。新增整链恢复回归，完整结果不再调用 AI，确认单个失败评审后只补该单元；45 项相关测试通过。旧隔离缓存逐项重建原请求哈希证明仅字段顺序不同后记录等价性审计，未改候选内容；真实恢复日志已确认 35 个前置单元全部 reused=true。此前因该问题重复扩展的结果保留在 superseded_units，未隐去调用记录。
- 最后两份清单分别为 `kangxi-content-task29-recovery-20260908/manifest-20260908T092210Z.json`、`kangxi-content-baseline-20260908/manifest-20260908T091444Z.json`。29 有 4 个单元缺失可复用返回；30 的 2 个失败分别是引文末尾时间多包含下一句、最后一个词证据为零时长且落在切点。未猜测修正，也未确认重试这 6 个单元。
- 对现有 10 个 AI 可出片样本及“重逢初恋”完成转写与抽帧初查，另生成独立本地预览；尚未完成逐段音画人工验收。初查发现 8–33 秒短片偏多、“一天只卖 30 盒”更像产品解释且爆点偏弱；初恋虽去掉无关开头，仍被基线前三秒要求淘汰。保留这些反例，不以数量或 AI 决定宣称改善，不再据此批量放宽。记录位于 `data/diagnostics/kangxi-content-inspection-20260908/content-inspection-notes.md`。PR #85 保持草稿，未部署、未恢复活动正文。

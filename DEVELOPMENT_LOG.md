...
单上传表单、FormData、API、模型与任务 JSON 贯通可选策略；缺字段的历史客户端保留原流程。新原片模式在字幕步骤之前暂停，不伪造字幕 checkpoint、不自动运行文案或发布；任务详情引导检查成片并阻止重复 auto-resume。同步发送中心遵循新策略，新增字幕缺少审核/验证时不回退原片；字幕页面取消重复跳过入口并处理缺失按钮。

...
- 设置页只保留完全离线 faster-whisper 与共用 Codex CLI 两组配置；模型固定展示 `gpt-6-astra`，分别说明转写离线和 Codex 联网运行，提供依赖、可用性及登录检查。
- 设置保存只更新有效字段，拒绝旧 Provider、云转写和未知字段；历史地址、模型、密钥及 Codex Home 保留且不回传。没有修改全局 OpenAI 提供商、登录方式或认证。
- Provider 构造、直接调用、诊断、转写、分析及重试入口统一阻止停用接口。旧实现与历史结果保留，旧任务需要明确选择当前流程，不能静默回退或混用检查点。
- 新增 15 项边界测试，并更新现有兼容实现的离线测试；集成完整回归 908 项通过。未调用真实模型、未修改运行配置或重启服务。

...
## 2026-08-29 Git 与 PR 工作流统一

- 仓库级 Git 指令改为继承全局安全流程：从真实默认分支创建 `codex/<类型>-<说明>`，明确路径暂存，普通 Push 后创建 PR。
- PR 必须记录验证、风险和回滚；只有 CI 通过且用户明确确认后才允许 Squash 合并。
- CI 新增最小 `contents: read` 权限，不修改产品逻辑、数据库、发布链路或真实平台边界。

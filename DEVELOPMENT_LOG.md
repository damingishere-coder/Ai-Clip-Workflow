...
## 2026-05-24 新增一键推送功能
- 左侧导航新增“一键推送”，并新增独立页面 `/publish`，不再把发布入口只放在字幕工作台右侧小区域。
- 新增 `publish_jobs` 数据表，用于保存抖音 / B站发布任务、本次选择的视频来源、标题、简介、标签、状态和错误信息。
- 新增 `/api/publish/jobs` 创建发布任务接口，支持同一条切片同时勾选抖音和 B站，并分别生成本地发布任务记录。
- 新增推送任务取消接口和状态更新接口，为后续真实平台上传、失败重试和人工确认预留稳定入口。
- 一键推送中心支持手动选择“原始切片”或“带字幕成片”；当前只创建本地待推送记录，不会直接发布到平台，避免误发。
- 字幕工作台右侧“后续推送配置”改为“去一键推送中心”入口。
- 已验证：`python -m compileall app` 通过。

## 2026-05-24 字幕实装提交确认
- 复核字幕 MVP 链路：`/subtitles` 任务列表、`/subtitles/{task_id}` 单任务字幕工作台、字幕样式保存、字幕状态落库、ASS 字幕文件生成、FFmpeg 烧录和带字幕视频预览接口均已接入。
- 新增 `scripts/test_subtitle_rendering.py`，覆盖字幕时间裁剪、ASS 时间格式、颜色格式和文本转义，避免后续改动破坏字幕生成基础逻辑。
- 同步更新字幕相关文档，把“下一步接入自动加字幕”的旧口径调整为“单条切片字幕 MVP 已接入，后续做批量、字幕编辑、打码和发布前确认”。

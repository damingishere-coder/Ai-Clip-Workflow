...
本地初次批次/审核测试 40 passed，手机两项发现布局溢出后定位修复；创建、pipeline/checkpoint、批次、待办和浏览器核心回归 94 passed。移动 header 改为纵排后，最终关联回归 112 passed、审片与创建浏览器/服务 28 passed；最终完整回归与部署记录保存在忽略目录 data/acceptance/review-ui/，生产数据库和已有配置不迁移、不改写历史字幕决定。

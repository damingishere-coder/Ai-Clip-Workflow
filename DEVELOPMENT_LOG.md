...
## 2026-09-08 按放宽后的 1 号重新分析第 29、30 条

- 用户要求重新跑第 29、30 条。正式代码拒绝覆盖已有切片引用/发送记录的候选：29 原任务 `7d20ae5ede35` 有 3 条 WAITING 发布记录，30 原任务 `05503de15ac1` 有 2 条切片。因此创建明确标注“29 复跑”“30 复跑”的独立运行记录，保留原任务和所有下游引用，没有删除或解绑旧记录来绕过保护。
- 创建前备份活动库至 `data/backups/rerun-29-30-20260908-135845`。复跑任务分别为 `bba2601f914d` / `4dd252a85058`，Job 为 `f4ad53a42944` / `a4dca1904b65`；视频、转写、音频复制到各自目录并逐一校验 SHA-256，使用现有任务服务创建，通过正式 auto 接口入队，串行运行。
- 两份新任务均冻结同一 `preset_001` 的第 3 次内容修订，SHA-256 为 `c27bcd8a8dd421019e1e19121eee304b70b4ca4b4c36cb12c03b047bf6181129`。复用转写，三个 AI 阶段全部按新标准重跑；旧标准下的 AI 单元没有复制进新 Job。Codex / gpt-6-astra 和登录配置保持不变。
- 第 29 条真实完成 18/18 单元、覆盖率 100%、失败及非法条目均为0。召回由17增至31个，扩展返回18条，最终候选由3增至12条（7A、5B）；按现有最多5条的设置自动出片5条，旧 AI 原先自动选中1条。FFprobe 检查5条均为 H.264 + AAC，时长80/81/73/67/79秒；任务停在 PENDING_SUBTITLE_REVIEW。
- 第 30 条真实完成 19/19 单元、覆盖率 100%、失败及非法条目均为0。召回由20增至34个，扩展返回18条，最终候选由3增至12条（3A、9B），自动出片由2增至3条。FFprobe 检查3条均为 H.264 + AAC、480×360，时长68/84/91秒；任务停在 PENDING_SUBTITLE_REVIEW。
- 两条复跑的活动分析分别为 `1f4a237cd12a` / `c1ab47eaade2`，北京时间14:17:27 / 14:38:52完成分析。8条新视频均检查时长、视频轨与音轨，字幕草稿已生成；目标复跑任务无发布记录。原29/30的任务、冻结规则、历史分析、候选、切片、发布记录和 Workflow Job 与备份逐项完全一致。
- 最终 SQLite integrity=ok、外键异常0，正式 deep readiness=ready。完整验收写入备份目录 `acceptance.json`；本轮只执行既有流程、更新开发文档，没有部署新代码，没有合并 PR，也没有修改模型/认证设置。更多候选仅证明覆盖增加，实际内容质量仍待人工审核。

## 2026-09-08 放宽康熙候选筛选，更新同一 1 号正文

- 用户确认当前选片过严。第 30 条只读回溯为：20 个召回、去重后 17 个、扩展 5 条、保留 3 条候选、2 条 A 级自动出片；数量收缩集中在扩展与最终评审，不能把转写证据不足直接当作内容不好。
- 只修改 `prompts/default_ai_prompt_preset_001.txt` 的业务规则：召回充分发现、扩展逐个保留可审核候选、最终评审独立评分。前三秒和15秒改为参考，允许必要铺垫、普通回应与中等候选；局部转写不清写明位置和限制，不虚构证据、不机械淘汰。保留连续切片、150秒上限和真实评分要求，周复盘补充同步消除与放宽标准冲突的旧硬约束。
- 未修改生产 Python/JS、结构化输出契约、78分 A 级阈值、笑点/完整度门槛或仅 A 级自动出片逻辑；阶段数量上限继续生效。提示词能够引导增加候选，实际增加多少仍须新任务验证，不以测试通过代替内容效果。
- 独立分支 `codex/relax-kangxi-selection` 基于正式 `c650128`，保留原工作区所有已有修改。71 项契约、综艺选择、不完整分析回归通过，8 条既有依赖弃用警告；副本验证三阶段完整收到 2925 字正文，更新只影响 1 号和新增的一条内容修订。
- 在无活动 Workflow Job 的前提下，先在线备份至 `data/backups/kangxi-prompt-relax-20260908-135211`，副本演练后通过正式 PATCH 接口保存同一 `preset_001`。正式内容修订为 `promptv_preset_001_003`，SHA-256=`c27bcd8a8dd421019e1e19121eee304b70b4ca4b4c36cb12c03b047bf6181129`；源文件与活动正文哈希一致，无需重启服务。
- 写后 SQLite integrity=ok、外键异常0；2/3/4号完整记录、原内容版本、任务冻结规则、历史分析、任务、Workflow Job、切片和发布记录与备份逐项一致。没有新建并列提示词，没有重新分析第 30 条，没有更改模型、登录、排期或投稿。备份目录的 `acceptance.json` 保存核验结果。

...
同一生产数据库副本、同一机器 profiling：页面上下文 6.764 → 0.354 秒，任务入口 10.909 → 0.244 秒，历史月历 4.318 → 0.059 秒，历史页 4.417 → 0.452 秒。该比较是隔离副本的服务计算，不冒充部署后的页面耗时。7 项新增性能/并发/初始化回归、56 项排期/历史/分组测试、78 项 API/就绪/围栏测试通过，Ruff、编译与 14 项 JS 语法通过。完整回归、CI、合并和原 RunDock 部署验收进行中；当前正式运行版本仍为下述 `3546e26`。

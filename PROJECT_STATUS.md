...
基于已部署 PR #124 / `30d9e6b`，重整成片验收区为状态/视频与下一步双栏。批次字幕直接读取带哈希验证的创建配置，确认接口可省略 delivery_mode，服务端拒绝审片时换模式；此前已有人工确认的不同模式保留。旧视频版本仍不可确认。单上传补充字幕选项并写入任务配置，新任务 original 在字幕步骤开始前停人工审片，不改变 checkpoint 顺序或历史缺省配置。任务详情避免误重跑选片，同步发送中心遵循创建策略。没有数据库迁移。

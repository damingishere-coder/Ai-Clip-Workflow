...
不新增数据库迁移、不调整 AI 或发布流程。定向功能测试首轮 16 项通过，补充递归/取消/错误路径的桌面与手机交互检查后进行完整回归。本修复的 PR、最终检查和原 8001 服务部署证据在验收后补记；以下 v2.6.0 发布记录仍为既有基线。

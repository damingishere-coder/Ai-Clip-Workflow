...
正式 HTTP：首页 6.444→0.409 秒，任务入口 9.487→0.276 秒，历史月历 4.560→0.065 秒，历史页 3.869→0.429 秒；页面读取期间并发健康请求 5.627→0.004 秒。真实浏览器首页完整加载 **16.804→0.764 秒**，任务入口 0.423 秒。任务入口 HTML 6.91 MB→0.286 MB，394 个视频元数据预加载降为 0。正式页面原先因最新 200 条限制仅显示 29 条排期，现显示完整 **32 条**，与调度/数据库一致。

浏览器实点排期清单、历史首/第二页（910 条、19 页、每页 50 条）、E1848 聚焦 3 条和“查看全部任务”恢复 7 组，通过且无 JS 错误、无横向溢出。此次只读验收未保存文案或改变排期。调度扫描连续推进、失败计数 0，Worker 正常；截图的历史瞬时异常未在当前版本复现，现存 RunDock ring log 没有调度 traceback，不能将 HTTP 断连的 WinError 10054 噪声认作调度失败。本次减少重复迁移及数据库写锁，保留初始化失败重试与正式发送门禁。

维护前在线备份 SHA256 `86AED4B40B053A015742920D37139AEC91535765DFF95772892A647B6CFE2827`，部署前后 **51 张表内容摘要完全相同**；所有已登记成片文件大小/修改时间和配置哈希不变。readiness=ready、23 项迁移验证、integrity=ok、外键异常 0；正式 JS 响应哈希匹配运行源码。无数据库迁移或配置变更，不触发 AI、成片确认或投稿。证据在 `Ai-Clip-Workflow-publish-performance/data/acceptance/performance/`；回滚代码保留 `3546e26`，无需用旧数据库覆盖后续用户操作。本条仅补文档，无需再次重启。

## 修复设计与副本性能：发送中心与调度扫描（2026-09-17）

正式环境只读基线：`/publish` 6.762 秒 / 1,559,794 字节，任务聚焦入口 13.141 秒 / 6,826,584 字节。页面重复重建已经没有模板消费者的旧队列，逐条校验大量历史记录；任务入口取消数量限制后还为全部历史生成隐藏编辑器和视频预加载。同步数据库与文件处理直接占用 ASGI 事件循环，连健康请求也会等待。

修复仅加载全部活跃准备/排期记录，历史记录留在独立分页入口；先过滤日期、分页，再处理当前页的就绪与脱敏。页面轮询按实际行 ID 分批刷新，覆盖最早的排期和刚进入终态的记录；视频不预取、封面延迟加载。相关读取路由转入线程池。调度器只在实例首次成功初始化数据库，失败仍重试，不再每 5 秒重复执行完整迁移与兼容数据更新。

同一生产数据库副本、同一机器 profiling：页面上下文 6.764 → 0.354 秒，任务入口 10.909 → 0.244 秒，历史月历 4.318 → 0.059 秒，历史页 4.417 → 0.452 秒。该比较是隔离副本的服务计算，不冒充部署后的页面耗时。7 项新增性能/并发/初始化回归、56 项排期/历史/分组测试、78 项 API/就绪/围栏测试通过，Ruff、编译与 14 项 JS 语法通过。最终完整回归、CI、合并与正式运行证据见最上方交付记录。

本次不修改数据库结构、配置、媒体或排期，不触发 AI、重新验收或平台投稿。性能证据和部署前后快照存放在本次工作树忽略目录 `data/acceptance/performance/`。
...
本次将同步放入线程池，明确同步中状态并锁定冲突操作；原视频策略的人工确认继续执行内容准备同步，失败保留确认并可重试。候选改选、时间编辑、源监视器、删除及保存均联动成片面板；改动后提示“保存选择并生成新版成片”，新版仍须人工验收。空选不能生成，过滤列表按任务总选中数计算。内容准备按 task_id 聚焦当前任务，可切回全部；排期和历史仍保留全局视图。已确认的当前版不再显示要求取消已有草稿的误导提示。

12 项既有 Chrome 审片回归和 5 项新增完整交接回归通过，包含 1440/390 补选重切、保留旧文件、旧草稿归档、新版 WAITING、重复同步新增 0、失败重试与慢同步期间健康请求可用。完整回归、最终 CI 和正式运行验收结果见本次 PR 及忽略目录 `data/acceptance/batch-review/DELIVERY.md`；当前记录不将开发完成等同部署。无数据库迁移或配置变更。本次正式操作只同步用户已确认的 E1848 3 条成片，没有确认其他新版本、排期或发布。
...
修复将已退役切片的未排期、未执行 DRAFT/WAITING 单独识别。GET 仅说明数量，用户确认当前版本时，在同一事务内将这些旧草稿转 CANCELLED、写入 superseded_by_recut 事件并保存新凭据，保留视频及成功历史。当前版草稿、已排期、尝试过发送或结果待复核的记录继续阻断；页面提供发送记录处理入口。字幕、排期和发布仍独立确认。

同时修复后台 init_db 每轮旧字段回填触发失效成片门禁的问题：现代素材批次不参与历史字段回填，旧任务只在源有值且确实变化时回填。无 DDL、配置或数据库版本变更。正式库只读快照完成两次隔离初始化，1059 条发送记录逐条一致。41 项成片回归及新增启动/调度用例通过；完整测试、CI、合并和部署结果将在验收后追加，不以开发完成推定上线。

## 排期请求初始化修复（2026-09-16）

基础请求函数改为随 HTML 初始化，发送中心不再因通用 app.js 加载失败而报 apiFetch 不存在。保留既有排期业务规则、预览确认门禁和历史记录。增加缺失脚本故障注入及真实 API 浏览器排期保存回归；工程验证、合并和部署事实以当前 PR 与 data/acceptance/schedule-client/DELIVERY.md 为准。
...
素材批次重新切片成功后，主任务原先误标 completed；现在同事务落为 pending_review，详情提示确认实际成片而非继续自动流程或直接发送；已整合 PR #125，字幕沿用创建时设置。“进入内容准备”同步成功后跳转当前任务；人工版本凭据、字幕验证、独立排期边界保留，无数据库迁移。
...
基于已部署 PR #125 / fcdc348，完成后台分析与交互切片队列拆分。已分析素材可在其他素材转写/分析时切片；同一素材仍不能同时修改产物，实际切片最多一路（包含自动预切），转写/分析后台通道最多一路。审片、内容同步与保存排期仅检查当前素材。切片期间离开再返回会恢复进度。

没有数据库迁移、配置变更或生产数据补写；保持人工成片/字幕审核和排期确认。最终回归、CI 与已有 RunDock Web 部署验收记录位于忽略目录 data/acceptance/interactive-cut/DELIVERY.md，完成前不将开发状态当作部署状态。

## 成片确认区与创建时字幕策略（2026-09-15）

基于已部署 PR #124 / `30d9e6b`，重整成片验收区为状态/视频与下一步双栏。批次字幕直接读取带哈希验证的创建配置，确认接口可省略 delivery_mode，服务端拒绝审片时换模式；此前已有人工确认的不同模式保留。旧视频版本仍不可确认。单上传补充字幕选项并写入任务配置，两种策略的新任务在字幕步骤开始前停人工审片，单上传也接入实际视频清单和人工确认门禁，不改变 checkpoint 顺序或历史缺省配置。任务详情避免误重跑选片，同步发送中心遵循创建策略。没有数据库迁移。

核心 94 项定向回归通过；最终关联守卫 112 项、审片与创建浏览器/服务 28 项通过，完整回归进行中，PR/CI/正式运行事实见本次 PR 和忽略目录 `data/acceptance/review-ui/DELIVERY.md`，不能以开发完成推定已部署。

## 统一待办整合记录（2026-09-15）

基于已部署 PR #123 / `1ceb0f6`，把独立待办整合为素材池内部视图，移除侧栏独立入口。原只读分类、分页及处理目标保留，旧URL兼容跳转。全部生产改动限于模板与静态资源，无 Python、配置或数据库迁移，可在不中断现有 AI 转写/分析的情况下部署。功能验证与 Git 发布证据以本次 PR 和忽略目录 `data/acceptance/material-inbox/` 的运行验收记录为准。


## 当前开发：素材池 UI 重整（2026-09-15）

素材卡片、左右工作区、精简制作参数、显式跳过字幕及最近批次进度卡片已实现。跳过字幕复用 original 交付预选，仍需人工审片；已有字幕保留，转写与发布门禁不变。没有数据库迁移或生产 Python 修改。本地六文件定向回归 60 passed；新增策略与两宽度浏览器增量 12 passed，Ruff 与两个 JS 语法检查通过。1918/390 只读页面预览确认卡片选择、字幕提示及无横向溢出。正在完成 PR/CI；正式部署事实在验收后补充。


## 当前：素材文件夹修复已部署（2026-09-15 14:42）

已定位原素材页只有路径输入、扫描不递归，年份目录会把月份文件夹全部排除。增加网页内本机文件夹选择窗口、默认勾选包含子文件夹、带相对路径的预览和空结果指引。扫描/登记上限为 1000 视频、全树 10000 项、20 层；任务批次仍最多 200 个。直属源目录身份保持不变，同一视频从年份或月份入口登记均复用；链接/重解析点、网络路径、目录替换、文件变化继续拒绝。

修复 [PR #121](https://github.com/damingishere-coder/Ai-Clip-Workflow/pull/121) 最终提交 `4deaf58` 的 Linux、Windows、Docker CI `34937627475` 全通过，Squash 合并为 `36ded1f`。本地完整回归 **1350 passed、0 failed**（519.01 秒）、Ruff、编译、JS 检查通过；1440/390 Chrome 验证选择器、递归切换、错误路径、取消和登记幂等。没有数据库迁移，版本仍为 2.6.0，此为后续修复提交。

14:40:44 更新原 RunDock Web 8001，运行目录仍为 `Ai-Clip-Workflow-offline-runtime`，HEAD 为 `36ded1f`；Worker 8765 保持原进程。两端监听父链与原服务记录匹配，deep readiness=ready、23 项迁移、integrity=ok/FK=0，实际 JS/CSS 响应哈希匹配新源码。正式页面实际点击磁盘 → 素材目录 → 年份 → 选择此文件夹 → 扫描：**12 个月份、260 个可登记视频、0 排除**，含月份相对路径，确认登记仍禁用至用户勾选，浏览器无脚本错误及整页横向溢出。

维护前 SQLite 在线备份保存在忽略目录 `data/acceptance/material-folder-fix/before.sqlite3`，并记录 SHA-256；51 张表在维护前后完整一致，正式扫描验收只新增 1 条 material_scans，其他 50 表未变。配置、外部原片与项目托管媒体元数据指纹不变，没有登记正式素材、创建任务、调用 AI 或提交发布。完整回归、截图、维护和监听/资源验收证据在同一忽略目录。回退基线为 `75cdd93`，不得恢复数据库来撤销预览或覆盖后续业务事实。后续仅交付文档，无需再次重启服务。
...
素材页追加完整导入哈希相同提示；未确认预览过期七天后由空闲维护分批清理，已登记证据和原片保留。新定向 17 项与两宽度 Chrome 2 项通过，完整回归继续；无新迁移，正式仍 v2.5.5 / c0d7962、19 项迁移。

审查补强：失败发布单列人工重试待办；手动导入/待 AI 显示“待继续处理”，首页注明数量。最终定向 8 passed（4.29 秒）。正式备份已隔离恢复，19→23 重复初始化通过，41 张旧表原字段/记录保留、完整性正常、外键异常 0；正式数据库未升级。

交付补强：失败/取消导入在素材池显示明确重试按钮，调用原 Job retry，保留任务/配置；Inbox 导入异常直接跳素材池。最终工作台 10 项通过，新增导入按钮浏览器检查继续。完整回归采集 1345 项执行至 100%，旧周复盘两个宽度复制按钮超时，保留失败并独立诊断；不得宣称完整全绿。PR #118 初次三项 CI 通过，补强后须重新验证最终提交。
...
8 项新增测试通过：十任务一项转写失败其余九项进入审片；子 Job 创建后注入失败原子回滚并恢复，复制不重做；预切 checkpoint 后退出恢复不重跑 AI 或切片；部分成片失败不伪装待审完成；四种越界续接拒绝。旧批次/Pipeline/checkpoint 55 passed；桌面/窄屏批次与审核浏览器 4 passed。全量回归与只读审查继续收口。无新 DDL，正式仍 v2.5.5 / c0d7962、19 项迁移。

最终补强：已有未取消发布任务时禁止替换交付确认；失效批次排期转 NEED_REVIEW，终态仍能落库。发送队列按确认的字幕模式处理，未确认项单列待审核；批次取消后新建符合新确认的发布任务，不先恢复旧字幕路径。最新发布/审核/分组 51 项通过，Ruff、compileall、13 JS 通过。
...
新容量/真实 OS 锁、原 fencing 和批次导入共 38 项定向通过；完整离线回归 1289 passed（334.79 秒），Ruff、编译及 12 JS 检查通过。独立只读审查未发现队列内阻塞。正式仍 v2.5.5 / `c0d7962`、19 项迁移；v2.6 的全自动预切/人工凭据、Review Inbox 和首页仍未交付，不提前启用自动生产。

## 已合并：v2.6 PR2 批次任务与原片导入（尚未部署）
...
## 2026-09-24 过期发布排期保护（待合并部署）

针对服务或账号恢复后集中补发过期任务的问题，调度器改为仅在计划时间后两分钟内允许正常扫描延迟；超过后将尚未开始上传的任务移回待排期，记录原时间及 `schedule_missed` 审计事件，不自动投稿或重排。后续未到点的任务保持原排期。发送中心显示错过数量、原计划时间和重新排期入口；用户保存新排期后清除提醒。已通过调度状态机、API、动态排期和真实 Chrome 页面回归；合并、正式运行更新及实际验收另以交付记录为准。

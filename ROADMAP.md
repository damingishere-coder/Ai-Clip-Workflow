# NiuMa Studio Roadmap

路线图用于说明牛马片场下一阶段的优先级，不代表固定交付日期。平台页面、外部 API 和本地环境变化可能影响顺序。

## 当前版本：2.5.0

当前目标是稳定 Windows 本地单用户生产闭环：

```text
导入长视频
→ 转写
→ AI 选片
→ 人工审核
→ 生成短视频
→ 准备平台内容
→ 排期或立即发送
→ 保存执行证据
```

## 已批准的下一阶段

严格按 **v2.4 Content Profile → 工程验收 → v2.5 多模态理解 → 工程验收 → v2.5.5 选片经验闭环 → 工程验收 → v2.6 内容生产队列** 推进。2026-09-14 用户批准真实质量在日常使用中补验，不再要求先提供专门素材或完成全部盲审；试用状态与人工审片/发布边界保留。继续使用 FastAPI、SQLite、本地文件和 Windows Worker，不引入新的队列基础设施。

- v2.4：五种内容模板、共享访谈/知识分析、不可变策略证据已接入 master；康熙三阶段、旧任务、旧 Prompt 和 checkpoint 保留。访谈/知识标试用，工程发布与真实质量结论分开记录，实际发布状态见 PROJECT_STATUS。
- v2.5：候选抽帧、可选视觉 Provider/checkpoint、辅助综合评审、证据页与缓存生命周期已接入 master；默认关闭并标试用。1187 项本地回归及三项 CI 通过，版本实机发布门禁进行中。
- v2.5.5：复用审片、官方指标和实验，生成有样本门槛的建议；生产策略只经人工确认改变。尚未开始。
- v2.6：目录素材预览、幂等批量任务、受限 Workflow Job 队列与统一审核 Inbox；保留原片与人工发布边界。尚未开始。

具体 PR、数据库变化及验收/回滚规则以 [实施账本](docs/CONTENT_PROFILE_ROLLOUT.md) 为准。目录批量导入属于 v2.6 目标，当前创建页仍为单文件上传。

## 2.3 已交付

- [x] PR #86 将既有离线转写、内容复盘、周总结、动态排期和 AI 恢复修复整合到 master。
- [x] 2.3.0 版本统一、GitHub 首页改版、历史分支收缩。
- [x] Windows 实机验收与发布门禁通过，v2.3.0 正式发布并部署到既有 RunDock 服务。

详细证据和未合并边界见 [项目当前进度](PROJECT_STATUS.md)。近期操作只维护在 [NEXT_STEPS.md](NEXT_STEPS.md)，旧路线图保存在 [历史快照](docs/ROADMAP_HISTORY.md)。

## 已完成的基础建设

- [x] 重构中英文项目主页与社区文档
- [x] 移除 Docker 中作者电脑的 E 盘硬编码
- [x] 拆分正式、开发和隔离 Demo 三套 Compose
- [x] 增加 `setup`、`doctor`、`start`、`stop` 脚本
- [x] 增加不连接真实账号的 Demo 数据
- [x] 增加 Python、JavaScript、PowerShell、Compose、敏感文件和 Demo CI 检查
- [x] 固定经过 CI 验证的直接依赖版本
- [x] 增加最终 Docker 镜像启动与页面冒烟测试
- [x] 建立 Dependabot 更新入口
- [x] 增加 SQLite、`.env`、媒体文件的备份、恢复与升级前保护（[#24](https://github.com/damingishere-coder/Ai-Clip-Workflow/issues/24)）
- [x] 增加恢复前回滚包、SHA-256 清单与损坏备份拒绝机制
- [x] 增加 Windows 云端主机冒烟、实机验收报告和正式版本发布门禁
- [x] 增加 `.env`、正式 SQLite 与正式任务目录的验收前后指纹保护
- [x] 统一 Docker / Windows 原生日常启停与发布 Worker 生命周期
- [x] 接入受控 Codex CLI 文本分析，保留远程 DeepSeek 与本地 Ollama 回退
- [x] 固定综艺笑点优先选片，并增加任务名称历史候选
- [x] 加固抖音文案规则、任务组全选、片段审核全选与异步切片防重复

## 当前公开任务

### P0：发布前可靠性

- v2.3.0 最终发布提交的 Windows 实机验收与发布门禁已完成；[历史验收 Issue #23](https://github.com/damingishere-coder/Ai-Clip-Workflow/issues/23) 仍开放，不代表 2.3 尚未验收。
- [抖音与 B站真实发布灰度验证矩阵](https://github.com/damingishere-coder/Ai-Clip-Workflow/issues/25)

### P1：展示与安装体验

- [补充 60 秒演示并持续更新已有真实截图](https://github.com/damingishere-coder/Ai-Clip-Workflow/issues/26)
- [拆分可选的本地 faster-whisper 依赖](https://github.com/damingishere-coder/Ai-Clip-Workflow/issues/27)

### P1：平台诊断

- [完善平台页面变化诊断与失败证据管理](https://github.com/damingishere-coder/Ai-Clip-Workflow/issues/28)

## 后续方向

### 内容生产质量

- 改进长内容分段分析、候选去重和全局排序。
- 增加不同内容类型的选片模板与可编辑评分规则。
- 完善标题、简介、话题和封面建议的人工确认流程。
- 加强片段版本、字幕版本和发布任务之间的关联可见性。

### 工程质量

- 完善现有迁移账本的兼容说明和升级恢复演练。
- 完善模块化 Publisher 接口和平台适配测试夹具。
- 提供可验证的 Windows 便携启动包或安装包。
- 继续补充英文安装、排错和发布文档。

## 暂不计划

以下能力不属于当前产品方向：

- 绕过验证码、短信、二维码、滑块、登录失效或平台风控。
- 完全无人监督地对真实账号无限重试或批量投稿。
- 默认暴露到公网的多用户 SaaS。
- 保存、共享或托管用户的平台账号密码和 Cookie。
- 自动抓取受访问控制、付费限制或版权限制保护的内容。

## 如何参与

欢迎通过 Issue 提交实际使用场景、失败复现、文档建议和平台页面兼容问题。提交前请阅读 [CONTRIBUTING.md](CONTRIBUTING.md) 和 [SECURITY.md](SECURITY.md)。

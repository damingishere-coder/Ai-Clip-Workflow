...
严格按 **v2.4 Content Profile → 验收 → v2.5 多模态理解 → 验收 → v2.5.5 选片经验闭环 → 验收 → v2.6 内容生产队列** 推进。继续使用 FastAPI、SQLite、本地文件和 Windows Worker，不引入新的队列基础设施。

- v2.4：五种内容模板、共享访谈/知识分析、不可变策略证据已接入 master；康熙三阶段、旧任务、旧 Prompt 和 checkpoint 保留。真实素材与人工质量验收尚未全部通过，不能作为正式版本完成。
- v2.5：文字召回后，仅对候选抽帧；视觉为可关闭、可降级的辅助证据。尚未开始。

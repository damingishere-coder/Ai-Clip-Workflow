"""Append the first story policy and Prompt without replacing user presets."""
...
VERSION = "20260914_02_interview_profile"
NAME = "人物访谈模板与独立提示词"
SEED = json.loads(Path(__file__).with_name("interview_profile_seed_v1.json").read_text(encoding="utf-8"))
CHECKSUM = hashlib.sha256(("interview-seed-v1:max-slot-plus-one:" + json.dumps(SEED, sort_keys=True)).encode()).hexdigest()
...
def verify(c):
    ...
    if not row or tuple(row) != (p["config_json"], p["config_sha256"]):
        raise ValueError("访谈 Profile 初始版本证据不一致")
    ...
    if not row or tuple(row) != (p["prompt_text"], hashlib.sha256(p["prompt_text"].encode()).hexdigest()):
        raise ValueError("访谈 Prompt 初始版本证据不一致")
    if c.execute("PRAGMA foreign_key_check").fetchall():
        raise ValueError("访谈模板迁移外键校验失败")

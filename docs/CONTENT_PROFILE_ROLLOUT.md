...
PR1 #93 已合并为 `4264e31`。PR2 接入旧三个模式 Registry、固定种子迁移、任务/新 Job/Run 证据；康熙关键参数从配置基线读取。旧任务的新执行冻结当次版本，旧 Job 恢复与历史版本未知保持原样；不匹配代码的规则版本拒绝执行。PR2 的数据库副本演练已通过，正式部署与最终验证尚待交付记录。后续三个 PR 及版本门禁不变。

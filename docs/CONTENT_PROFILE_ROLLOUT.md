...
本文记录已批准的实施边界；目标能力不等于已上线能力。当前工程版本为 2.4.0，实际发布/部署状态以 PROJECT_STATUS 为准。
...
PR1 #93 合并 `4264e31`，PR2 #94 合并 `cc8ae63`，PR3 #95 合并 `06d4d12`，PR4 #96 合并 `a5ece66`，PR5 #97 合并 `4eaff62` 并部署，12 条迁移、旧数据与 Worker 正常。五 Profile、创建页、Prompt/Provider 选择和 Judge 反馈快照已接入；followup 复用保持旧快照兼容，不改变评分/Prompt/旧指纹。PR6 #98 从质量证据草稿继续收口工程发布：试用标记、版本统一和实机门禁；真实质量保留待验，不再作为下一版工程开发的前置条件。

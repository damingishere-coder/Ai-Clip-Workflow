...
## 正式数据库当前状态（2026-09-15 14:03）

正式迁移账本为 23 项。迁移 20–23 曾由 13:26 临时诊断提前应用，原因与修正详见 PROJECT_STATUS；41 张迁移前旧表数据保留、9 张新表空。随后维护前快照的 50 张表在正式服务切换后全部保留原字段/记录，完整性正常、外键异常 0。

已用 c0d7962 在隔离副本验证当前空新表时可初始化，未修改任何行；其迁移执行器没有一概拒绝更高账本。不要将此推广为支持 v2.6 批次或任意降级。回滚必须保存当前发布事实，并针对实际数据验证；不删除账本、不无条件覆盖旧备份。

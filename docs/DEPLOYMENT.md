...
v2.4.0 支持两种运行方式。日常使用推荐 Windows 原生模式；真实抖音 / B站投稿与抖音作品指标同步无论使用哪种方式，都必须由 Windows 主机上的 Chrome Worker 执行。
...
**适用场景**：隔离 Demo、集成验收与已配置的 Docker 回退环境。维护者当前日常服务使用下方原生方式；已有托管服务时不要另起容器抢占端口。

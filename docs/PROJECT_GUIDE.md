...
创建任务时先选内容模板和 Prompt。若当前使用 Codex，可按需勾选“视觉辅助（试用）”；不勾选仍按原文字/音频链路处理。开启后只查看候选范围内少量图片，不上传整段视频。审片或 AI 分析历史中的“视觉证据”可以查看关键帧时间、观察、限制和失败原因；图片被清理后文字证据仍在，不会自动重新调用模型。

项目不会绕过二维码、短信、验证码、滑块、登录失效或平台风控。真实发送前必须人工核对内容与账号。

### 在使用中积累选片经验

审片时可展开“评价 AI 初始候选”，明确接受或拒绝并填写原因；普通启用/取消勾选仍用于生产选择。进入“内容复盘”可保存经验报告，无官方数据时只看人工统计，样本不足会直说。报告里可以编写未验证的 Challenger 草稿，核对差异后创建逐步试验任务。

官方数据达到页面显示的门槛后，可选择冻结报告建立作品实验，在发送中心关联确实使用该策略的作品。实验记录“保留”仍不会更新正式方案；只有再点击“预览正式启用”、核对正文并确认才会影响后续新任务。已有任务保持冻结，误操作可在同一实验卡预览回退。无需为了开始使用先准备专门测试素材。

### 批量生产与统一待办

1. 打开“素材池”，粘贴本机素材文件夹路径，扫描后勾选视频并确认登记。只扫描当前文件夹，外部原片保留。
2. 在已登记素材中勾选本批素材，选择内容类型、Prompt、Provider、数量与字幕策略。需要自动处理时勾选“导入后自动转写、分析和预切”，然后核对并创建批次。
3. 导入和处理逐个进行。重复提交同一请求会复用批次；要再次生产同一素材，必须明确勾选“创建新任务”。浏览器请求中断后可重试同一批次。
4. 打开“统一待办”。手动批次可从“待继续处理”进入任务；自动批次预切完成后在“待审片”查看实际成片。确认前不会进入内容准备或排期。
5. 核对成片并选择字幕方式。需要新增字幕时先完成字幕审核与验证，再明确进入内容准备。修改候选、分析或成片后需要重新确认。
6. 在发送中心继续核对文案、账号与排期。失败发布和结果不确定会出现在待办中，由你决定重试或复核。

首页“可复盘作品”是已有官方数据中归因明确且核心指标齐全的独立作品数，不代表实验样本已经足够。真实选片质量仍通过日常使用补验。

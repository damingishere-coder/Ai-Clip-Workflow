...
功能 PR #105–#109 和版本 PR #110 已合并；最终提交 `c0d7962` 的 PR/主干 CI、Windows 11 + Docker Desktop 17 项实机门禁和原 RunDock 服务验收通过。2026-09-15 10:08 Web/Worker 均为 2.5.5，正式库 19 项迁移、38 张旧表记录/字段保留，配置/存储保护通过；Release 已发布为 Latest。有效本机证据在 `data/acceptance/v255-release-tests/windows-ps7-attempt2/`，首次报告因未指向实际 DB 已被额外校验拒绝，不能用于发布。

备份恢复、实际资源及桌面/390px 页面均验证；正式库不造报告/实验数据，完整决策流程使用隔离夹具。详细事实维护在 PROJECT_STATUS.md。以下保留可复用核对模板：未勾选项不能视作本次已执行，Issue #23/#25、源码下载和额外平台灰度不因本次发布自动修改。

...
→ 保存字幕样式
→ 单条切片自动加字幕
→ 生成 06_subtitled 下的 ASS 字幕文件和带字幕 MP4
→ 人工预览带字幕成片
→ 下一步接入批量字幕 / 字幕编辑 / 打码 / 一键推送
```

注意：现有数据库任务状态仍沿用 `completed` / `completed_with_errors` 表示“自动切割阶段结束”。字幕状态单独写入 `subtitle_jobs`，不直接混进任务主状态；平台发布能力后续再用发布任务表承接。

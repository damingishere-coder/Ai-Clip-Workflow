...
PR #100/#101/#102 已完成采样、证据、集成；发布 PR #103 合并 `384db79`，v2.5.0 已发布并于 2026-09-14 22:45 部署到原 Web/Worker。最终 1187 项本地回归、PR/主干三项 CI、Windows 实机及运行验收通过。正式库迁移 14、旧 37 表数据保留，所有旧任务视觉关闭。下列前序 PR 条目为历史过程；完整证据见 [项目进度](../PROJECT_STATUS.md)。
...
优先通过任务入口与 `AI_VISUAL_ENABLED=false` 关闭视觉，保留 2.5 二进制、增量表/列、Run 与 checkpoint。活动 Job 要先到安全点；不确定模型结果不补发，未确认停止的进程仍需人工处置。旧 2.4 对 14 项迁移库能初始化与读取旧任务，但 readiness 因两条未知迁移报错，已在独立副本实测；不能删账本伪造健康，也不能把旧备份覆盖回去视为无损。恢复整库前必须停服务、保存当前数据并核对新增事实与外部发布结果。

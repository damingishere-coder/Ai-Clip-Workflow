...
Assert-Gate ($mainText -match 'version="2\.6\.0"') '应用版本' 'app/main.py 不是 2.6.0。'
Assert-Gate ($readmeText -match 'version-2\.6\.0') '中文 README 版本' '版本徽章不是 2.6.0。'
Assert-Gate ($englishReadmeText -match 'version-2\.6\.0') '英文 README 版本' '版本徽章不是 2.6.0。'
Assert-Gate ($changelogText -match '(?m)^## 2\.6\.0 - ') 'Changelog' '缺少 2.6.0 正式记录。'

Write-Host ''
Write-Host '=== v2.6.0 发布门禁通过 ===' -ForegroundColor Green
Write-Host '下一步：核对 docs/RELEASE_CHECKLIST.md，然后创建 tag v2.6.0 和 GitHub Release。'

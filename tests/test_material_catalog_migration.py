...
from app.db import database as db, material_catalog_migration as migration
...
@pytest.fixture
def old_material_database(human_db, monkeypatch):
    path = human_db.parent/'old-materials.sqlite3'
...
def test_material_upgrade_preserves_old_tasks_and_restore_point(old_material_database):
    path,before = old_material_database
    db.init_db()
    db.init_db()
    with db.get_connection() as c:
    ...
    backups = list((path.parent/'backups').glob('*material-catalog-v1*.sqlite3'))
    ...
    with sqlite3.connect(backups[0]) as original, sqlite3.connect(path.parent/'restored.sqlite3') as restored:
        ...
        assert not restored.execute("SELECT 1 FROM sqlite_master WHERE name='source_materials'").fetchone()
        assert restored.execute("SELECT task_name FROM tasks WHERE id='legacy'").fetchone()[0] == '旧康熙任务'


def test_material_failed_migration_is_atomic_then_concurrent_retry_once(old_material_database, monkeypatch):
    original_apply = migration.apply
    def fail(c):
        original_apply(c)
        raise RuntimeError('material-injected-failure')
    with monkeypatch.context() as scope:
        scope.setattr(migration, 'apply', fail)
        with pytest.raises(db.SchemaMigrationError, match='material-injected-failure'):
            db.init_db()
    with db.get_connection() as c:
        assert not c.execute("SELECT 1 FROM sqlite_master WHERE name='source_materials'").fetchone()
